...
class FCNMaskTransformerHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(FCNMaskTransformerHead, self).__init__(init_cfg)
        ...
        self.patch_embeddings = nn.Conv2d(in_channels=self.conv_out_channels,out_channels=960,kernel_size=2,stride=2)
        self.position_embeddings = nn.Parameter(torch.zeros(1, 49, 960))
        self.dropout = nn.Dropout(0.1)
        self.up = Up(960+256, 256)
        self.transformer = Encoder(hidden_size=960, num_layers=6, mlp_dim=1536, dropout_rate=0.1, num_heads=8, attention_dropout_rate=0.0)
    ...
    def init_weights(self):
        super(FCNMaskTransformerHead, self).init_weights()
    ...
    def forward(self, x):
        for conv in self.convs:
            x = conv(x)
        t = self.patch_embeddings(x)
        t = t.flatten(2)
        t = t.transpose(-1, -2)
        t = t + self.position_embeddings
        t = self.dropout(t)
        t,_ = self.transformer(t)
        n, patch, dim = t.size()
        h, w = int(np.sqrt(patch)), int(np.sqrt(patch))
        t = t.permute(0, 2, 1)
        t = t.contiguous().view(n, dim, h, w)
    ...
    def loss(self, mask_pred, mask_targets, labels):
        ...
        Example:
            ...
            >>> self = FCNMaskTransformerHead(num_classes=C, num_convs=1)
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
        ...
        Example:
            ...
            >>> C, H, W = 11, 32, 32
            >>> # Create example instance of FCN Mask Head.
            >>> self = FCNMaskTransformerHead(num_classes=C, num_convs=0)
            >>> inputs = torch.rand(N, self.in_channels, H, W)
            >>> mask_pred = self.forward(inputs)
